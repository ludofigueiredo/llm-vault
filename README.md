...
For **single conversation exports**, `artefacts/` and `contenu/` are populated automatically: `artefacts/` contains any Claude-generated artifacts attached to the conversation (unzipped from a captured Blob when there are several, or fetched directly by a best-effort guessed filename when there's only one — that guess can occasionally be wrong, in which case the folder stays empty for that artifact), and `contenu/` contains any individually-attached files — both images and other file types (like `.pptx`) are fetched directly by URL. This only works for the conversation currently open in your active browser tab, and non-image files are only found if they were uploaded by you (Claude-generated output files attached to the conversation aren't covered yet). If a conversation has no artifacts or no attached files, the corresponding folder stays empty.
