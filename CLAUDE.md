...
2. If found, locates its download button by whitespace-normalizing each candidate button's `textContent` before matching (claude.ai renders "Tout télécharger"'s two words across separate text nodes/lines, so a raw substring check never matches). **"Tout télécharger" only renders when a conversation has more than one artifact** — with exactly one, there is no such button, so the code falls back to that single artifact's own `button[aria-label^="Télécharger "]`, extracting the artifact's name from the label for later use.
3. Before clicking, arms `extension/main-world-hook.js` via `window.postMessage` and awaits an `ARM_CAPTURE_ACK` reply (bounded, ~2s) confirming the hook is installed and listening — required because `postMessage` is asynchronous while claude.ai's click handler calls `URL.createObjectURL` synchronously, so clicking immediately after posting (without waiting for the ack) loses the race and the Blob is generated before the hook is armed. The hook runs in the page's MAIN JS world, necessary to intercept `createObjectURL` calls made by claude.ai's own React code, which an isolated-world content script cannot observe.
4. Awaits the hook's captured Blob (as an ArrayBuffer, since raw Blobs aren't reliably structured-cloneable across the `chrome.runtime` messaging boundary) with a ~10s timeout, then closes the sidebar again (only if this capture opened it).
...
6. For non-image files in that same section (rendered as `[data-testid="file-thumbnail"]` buttons with no `<img>`, e.g. `.pptx`), only the filename is scraped from the thumbnail's `<h3>` — these files do **not** use `URL.createObjectURL` at all (confirmed via a live Network capture: clicking their preview modal's "Télécharger" button issues a real `GET` navigation to `/api/organizations/{orgId}/conversations/{conversationId}/wiggle/download-file?path=...` with `Content-Disposition: attachment`, so the MAIN-world hook never fires and would time out every time). `sidepanel.js` instead reconstructs this URL directly from the scraped filename, assuming the file lives under `/mnt/user-data/uploads/<filename>` (the path for files the user uploaded, which is what this section shows — Claude-generated output files live under `/mnt/user-data/outputs/` instead and are not covered by this section), and adds it to the same `{filename, url}` list as image content files so both are fetched identically.

`extension/lib/zipBuilder.js` then unzips the captured artifact Blob (via JSZip, preserving its internal folder structure) into `artefacts/` — or, if `JSZip.loadAsync` rejects (a single-artifact download is the artifact file itself, e.g. raw `.html`, not a zip), writes the Blob directly under the extracted artifact filename — and fetches every entry in the merged content-files list (`credentials: 'include'`) into `contenu/`, skipping individual failures without aborting. Every step degrades gracefully — a missing button, missing section, capture timeout, or failed fetch never fails the overall export, it just leaves the corresponding folder (partially) empty. A real Chrome download is still triggered on disk when the artifact download button is clicked (same as manual use) — this is an accepted side effect of using real UI interaction, not a bug.
