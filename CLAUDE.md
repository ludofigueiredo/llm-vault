...
2. If found, locates its "Tout télécharger" button by whitespace-normalizing each candidate button's `textContent` before the substring match (claude.ai renders the label's two words across separate text nodes/lines, so a raw `.includes('Tout télécharger')` check never matches — this was the root cause of `artefacts/` always coming back empty) — but first arms `extension/main-world-hook.js` via `window.postMessage` (the hook runs in the page's MAIN JS world, necessary to intercept `URL.createObjectURL` calls made by claude.ai's own React code, which an isolated-world content script cannot observe).
...
5. For non-image files in that same section (rendered as `[data-testid="file-thumbnail"]` buttons with no `<img>`, e.g. `.pptx`), clicks each thumbnail to open claude.ai's file-preview modal, polls (bounded, ~3s) for its "Télécharger" button, arms the same MAIN-world Blob hook, clicks it, awaits the captured Blob (~10s timeout), then clicks the modal's `[aria-label="Fermer"]` close button before moving to the next file — one capture cycle per non-image file, sequential (not parallel, since only one Blob capture can be armed at a time).

`extension/lib/zipBuilder.js` then unzips the captured artifact Blob (via JSZip, preserving its internal folder structure) into `artefacts/`, and writes both the fetched image content files and the captured non-image content file Blobs into `contenu/`, skipping individual failures without aborting. Every step degrades gracefully — a missing button, missing section, capture timeout, or failed fetch never fails the overall export, it just leaves the corresponding folder (partially) empty. A real Chrome download is still triggered on disk when "Tout télécharger" or a modal's "Télécharger" is clicked (same as manual use) — this is an accepted side effect of using real UI interaction, not a bug.
