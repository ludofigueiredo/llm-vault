...
LLM Vault does not collect, store, transmit, or sell any user data. It runs entirely inside your browser and only communicates with claude.ai.

## What the extension does

LLM Vault is a Chrome extension that exports Claude.ai conversations and Projects into structured Markdown `.zip` files, downloaded to your own computer.
...
- **Page content**: On claude.ai pages, the extension's content script reads on-page text and image URLs (e.g. a Project's Memory/Instructions text, attached image thumbnails) that aren't exposed through claude.ai's API, so they can be included in your export.
- **No third-party transmission**: The extension does not send any data to any server other than claude.ai. There is no analytics, telemetry, tracking, or remote logging of any kind.
...
See the "Justifications for permissions" section of the Chrome Web Store listing for a full breakdown of why each requested permission (`downloads`, `activeTab`, `cookies`, `sidePanel`, `tabs`) is needed — each is used solely to read claude.ai page/session data and save the resulting export file locally.
