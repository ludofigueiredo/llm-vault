...
- **`scripting`**: Required to re-inject the extension's content script into the active claude.ai or chatgpt.com tab if it becomes unavailable (for example, if the tab was already open before the extension was installed or reloaded). This lets features like project/conversation selection and export recover automatically without asking the user to manually refresh the page. Used only on the two host permissions already requested (claude.ai and chatgpt.com) and never on any other site.
